...
> instruction to take its colors from the app theme is withdrawn: the ink and the
> page frame no longer follow the reader's theme, palette or dark mode, while the
> language and the fonts it resolves to still do. 0.9.3 injects MCP schema discipline into every plugin
...
> plugin: Task matchers, OMP executor guards, worker brief guards, and delegation operating guidance now belong to each harness's user
